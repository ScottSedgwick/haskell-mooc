...
firstHalf xs = take (ceiling (fromIntegral (length xs) / 2.0)) xs
...
capitalize xs = unwords (map f (words xs))
  where
    f [] = []
    f (x:xs) = toUpper x : xs
...
powers :: Int -> Int -> [Int]
powers k max = 1 : unfoldr f k
  where
    f x | x > max = Nothing
        | otherwise = Just (x, x * k)
...
multiCompose fs x = foldr (\a b -> a b) x fs
...
interpreter commands = reverse $ snd $ foldl f ((0, 0), []) commands
  where
    f :: ((Int, Int), [String]) -> String -> ((Int, Int), [String])
    f ((x,y), ps) "up"     = ((x, y + 1), ps)
    f ((x,y), ps) "down"   = ((x, y - 1), ps)
    f ((x,y), ps) "left"   = ((x - 1, y), ps)
    f ((x,y), ps) "right"  = ((x + 1, y), ps)
    f ((x,y), ps) "printX" = ((x, y), show x : ps)
    f ((x,y), ps) "printY" = ((x, y), show y : ps)
    f ((x,y), ps) _        = ((x, y), ps)
