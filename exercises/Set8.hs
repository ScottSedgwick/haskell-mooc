...
combine f (Picture g) (Picture h) = Picture (\c -> f (g c) (h c))
...
instance Transform Fill where
  apply (Fill c) (Picture _) = Picture (\_ -> c)
...
instance Transform Blur where
  apply Blur (Picture f) = Picture g
    where
      g (Coord x y) = blendColors [clr1, clr2, clr3, clr4, clr5]
        where
          clr1 = f (Coord (x-1) y)
          clr2 = f (Coord x y)
          clr3 = f (Coord (x+1) y)
...
blendColors cs = Color (getAvg getRed) (getAvg getGrn) (getAvg getBlu)
  where
    getAvg f = sum (map f cs) `div` length cs
    getRed (Color r _ _) = r
    getGrn (Color _ g _) = g
    getBlu (Color _ _ b) = b
