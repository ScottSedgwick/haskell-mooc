...
inputInOutput input output = forAll (elements input) (\x -> length (filter (\(a,b) -> a == x) output) > 0)
...
outputInInput input output = forAll (elements output) (\(a,b) -> length (filter (==a) input) == b)
...
frequenciesProp freq input = sumIsLength xs ys .&&. inputInOutput xs ys .&&. outputInInput xs ys
  where
    xs = getNonEmpty input
    ys = freq xs
...
genList = do
  n <- elements [3..5]
  fmap sort (vectorOf n (choose (0,10)))
...
instance Arbitrary Arg where
  arbitrary = oneof [genNumber, genVariable]
    where
      ...
      genVariable = Variable <$> elements ['a','b','c','x','y','z']

instance Arbitrary Expression where
  arbitrary = oneof [genPlus, genMinus]
    where
      genPlus  = Plus  <$> arbitrary <*> arbitrary
      genMinus = Minus <$> arbitrary <*> arbitrary
