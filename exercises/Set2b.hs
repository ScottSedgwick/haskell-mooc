...
leftpad s x = replicate (x - length s) ' ' ++ s
...
countdown x = "Ready! " ++ concatMap f (reverse [1..x]) ++ "Liftoff!"
  where
    f x = show x ++ "... "
...
smallestDivisor x = f (find g [2..x])
  where
    f Nothing  = x
    f (Just y) = y
    g y = x `mod` y == 0
...
biggestPrimeAtMost x = f (find isPrime (reverse [1..x]))
  where
    f Nothing  = 1
    f (Just y) = y