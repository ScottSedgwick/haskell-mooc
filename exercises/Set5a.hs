...
totalPrice (MkShoppingEntry _ c n) = c * fromIntegral n 
...
buyOneMore (MkShoppingEntry s c n) = MkShoppingEntry s c (n + 1)
...
toBin 0 = O End
toBin 1 = I End
toBin x = if x `mod` 2 == 0 
          then O (toBin (x `div` 2))
          else I (toBin (x `div` 2))
