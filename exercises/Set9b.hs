...
prettyPrint n xs = (intercalate "\n" (map printLine [1..n])) <> "\n"
  where 
    printLine r = map (printSquare r) [1..n]
    printSquare r c = if (r,c) `elem` xs then 'Q' else '.'
...
danger x [] = False
danger x (y:ys) = sameRow x y || sameCol x y || sameDiag x y || sameAntidiag x y || danger x ys
...
prettyPrint2 n xs = (intercalate "\n" (map printLine [1..n])) <> "\n"
  where 
    printLine r = map (printSquare r) [1..n]
    printSquare r c = if (r,c) `elem` xs 
                      then 'Q' 
                      else if danger (r,c) xs 
                           then '#'
                           else '.'
...
fixFirst n [] = Just []
fixFirst n ((i,j):xs) | j > n           = Nothing
                      | danger (i,j) xs = fixFirst n ((nextCol (i,j)):xs)
...
step sz xs = case fixFirst sz xs of
...
finish sz xs = if length ys > sz then tail ys else finish sz ys
  where
    ys = step sz xs
