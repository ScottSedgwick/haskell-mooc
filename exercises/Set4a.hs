...
longer :: Ord a => [a] -> [a] -> [a]
longer as bs | length as > length bs = as
             | length as < length bs = bs
             | otherwise = if head as < head bs then as else bs
...
incrementKey c= foldr (\(k,v) b -> (if c == k then (k, v+1) else (k,v)) : b) []
...
transfer from to amount bank | amount < 0 = bank
                             | otherwise  = f fromBal toBal
  where
    fromBal = Map.lookup from bank
    toBal   = Map.lookup to bank
    f Nothing _ = bank
    f _ Nothing = bank
    f (Just b1) (Just b2) | b1 - amount < 0 = bank
                          | otherwise = Map.insert from (b1 - amount) (Map.insert to (b2 + amount) bank)
...
maxIndex = fst . last . sortOn snd . assocs
