...
add a (Set []) = Set [a]
add a (Set xs) = if a `member` (Set xs) then Set xs else Set (sort (a:xs))
...
data State = Start | HasEggs | HasEggsAndFlour | HasEggsAndSugar | HasFlourAndSugar | HasAllIngredients | IsMixed | Error | Finished
  deriving (Eq,Show)
...
step HasEggsAndFlour   AddSugar = HasAllIngredients
step HasEggsAndSugar   AddFlour = HasAllIngredients
step HasAllIngredients Mix      = IsMixed
...
reverseNonEmpty (x :| []) = x :| []
reverseNonEmpty (x :| ys) = last ys :| reverse (x : init ys)
...
passwordAllowed s (ContainsSome t)   = any (\c -> c `elem` s) t
passwordAllowed s (DoesNotContain t) = all (\c -> c `notElem` s) t
